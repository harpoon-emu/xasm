...
public:
	void operator()(const ast::instruction::nullary &x) const {
		std::cout << std::hex << x.address_mode << ": " << std::hex << std::setfill('0') << std::setw(2) << x.opcode << std::endl;
	}

	template<typename T>
	void operator()(const ast::instruction::unary<T> &x) const {
		std::cout << std::hex << x.address_mode << ": " << std::hex << std::setfill('0') << std::setw(2) << x.opcode << " " << std::hex << std::setfill('0') << std::setw(sizeof(T) * 2) << static_cast<std::uint16_t>(x.operand) << std::endl;
	}
};
